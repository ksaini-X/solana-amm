...
use anchor_spl::{
    token_interface::{Mint, TokenAccount, TokenInterface}, // Add TokenInterface here
};
...
pub mod amm {
    ...
    pub fn provide_liquidity(
        ...
        let lhs = (token_a_amount as u128)
            .checked_mul(pool.token_b_reserves as u128)
            .unwrap();
        let rhs = (token_b_amount as u128)
            .checked_mul(pool.token_a_reserves as u128)
            .unwrap();
        ...
        let lp_amount = token_a_amount
            .checked_mul(ctx.accounts.pool.lp_token_supply)
            .unwrap()
            .checked_div(ctx.accounts.pool.token_a_reserves)
            .unwrap();
